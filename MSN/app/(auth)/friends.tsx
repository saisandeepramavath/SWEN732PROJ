...
const Page = () => {
	...
	useEffect(() => {
		...
		const createdQuery = firestore()
		  .collection('groups')
		  .where('createdBy', '==', user.uid);
	  
		const memberQuery = firestore()
		  .collection('groups')
		...
		const handleSnapshot = (snapshot: any) => {
		  snapshot.forEach((doc: any) => {
			allGroups[doc.id] = { id: doc.id, ...doc.data() };
		  });
		...
		unsubscribes.push(
		  createdQuery.onSnapshot(handleSnapshot, handleError),
		  memberQuery.onSnapshot(handleSnapshot, handleError)
		);
